$(document).ready(function () {
  ...
  window.updateChartData = function (event, index) {
    ...
    document
      .getElementById("stat-" + statLabels[index])
  ...
  $("#pokemonSearchInput").on("input", function () {
    ...
    let filteredItems = initialPokemonListItems.filter((item) =>
  ...
  $("#metaSearchInput").on("input", function () {
    let query = $(this).val().toLowerCase();
    let filteredItems = initialMetaListItems.filter((item) =>
      $(item).text().toLowerCase().includes(query)
    );
  ...
  function toShowdownSpread(statsSpread) {
    ...
    let evSpreadArr = [];
    if (evs[0] !== "0") {
      evSpreadArr.push(evs[0] + " HP");
    }
    if (evs[1] !== "0") {
      evSpreadArr.push(evs[1] + " Atk");
    }
    if (evs[2] !== "0") {
      evSpreadArr.push(evs[2] + " Def");
    }
    if (evs[3] !== "0") {
      evSpreadArr.push(evs[3] + " SpA");
    }
    if (evs[4] !== "0") {
      evSpreadArr.push(evs[4] + " SpD");
    }
    if (evs[5] !== "0") {
      evSpreadArr.push(evs[5] + " Spe");
    }
  ...
  function changeNature(currentNatureVal, stat, diff) {
    ...
    if (diff === "+") {
      if (stat === "Atk") {
        naturesForStat = attackNatures;
      }
      if (stat === "SpA") {
        naturesForStat = spAttackNatures;
      }
      if (stat === "Spe") {
        naturesForStat = speedNatures;
      }
      if (stat === "Def") {
        naturesForStat = defenseNatures;
      }
      if (stat === "SpD") {
        naturesForStat = spDefenseNatures;
      }
      if (attackNaturesM.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          attackNaturesM.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (defenseNaturesM.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          defenseNaturesM.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (spAttackNaturesM.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          spAttackNaturesM.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (spDefenseNaturesM.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          spDefenseNaturesM.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (speedNaturesM.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          speedNaturesM.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
    ...
    if (diff === "-") {
      if (stat === "Atk") {
        naturesForStat = attackNaturesM;
      }
      if (stat === "SpA") {
        naturesForStat = spAttackNaturesM;
      }
      if (stat === "Spe") {
        naturesForStat = speedNaturesM;
      }
      if (stat === "Def") {
        naturesForStat = defenseNaturesM;
      }
      if (stat === "SpD") {
        naturesForStat = spDefenseNaturesM;
      }
      if (attackNatures.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          attackNatures.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (defenseNatures.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          defenseNatures.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (spAttackNatures.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          spAttackNatures.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (spDefenseNatures.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          spDefenseNatures.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      if (speedNatures.includes(currentNatureVal)) {
        const filtered = naturesForStat.filter((n) =>
          speedNatures.includes(n)
        );
        return filtered.length ? filtered[0] : naturesForStat[0];
      }
      return naturesForStat[0];
  ...
  $(".export-button").click(function () {
    ...
    if (typeof exportData.ev === "string") {
      let stat = "";
      if (exportData.ev.includes("Atk")) {
        stat = "Atk";
      }
      if (exportData.ev.includes("SpA")) {
        stat = "SpA";
      }
      if (exportData.ev.includes("Spe")) {
        stat = "Spe";
      }
      if (exportData.ev.includes("Def")) {
        stat = "Def";
      }
      if (exportData.ev.includes("SpD")) {
        stat = "SpD";
      }
      let diff = " ";
      if (exportData.ev.includes("+")) {
        diff = "+";
      }
      if (exportData.ev.includes("-")) {
        diff = "-";
