...
- **Draft Scout** (`/tools/draft/`) — scout a draft-league opponent's whole roster at once instead of checking Pokémon one at a time. Two rosters (yours and theirs), and two questions answered across all of them
  - **Move coverage:** who has Fake Out, priority, speed control, screens, redirection, hazards…? Curated preset groups plus free-text search over every move and ability. **A preset expands into moves *and* abilities together** where both answer the same question — Weather brings Drought, Drizzle, Sand Stream and Snow Warning alongside Sunny Day, so asking "who sets sun" finds Torkoal, which no move list would. Priority pulls in Prankster/Gale Wings/Triage, Redirection pulls in Lightning Rod and Storm Drain, and Terrain pulls in the surge abilities; a preset carrying abilities is marked with a dot. Results are a **list, not a matrix** — only matches are printed, with everything that matched nothing collapsed onto one line at the end. A twelve-ability preset against ten Pokémon is a 120-cell grid that is almost entirely blank; the same query as a list is usually two or three lines. Groupable by move ("who has Fake Out") or by Pokémon ("what does this bring"), with the old grid still available via `?view=grid`. The Priority group is *derived* from `moves.json` (`priority > 0`) rather than hand-listed, so it cannot fall behind a new generation; moves whose priority is conditional (Grassy Glide) are folded back in explicitly, since the static field reads 0 for them
  ...
  - **Speed abilities are costed automatically**, on both sides and in the ladder: Swift Swim, Chlorophyll, Sand Rush, Slush Rush, Surge Surfer, Unburden, Quick Feet, Slow Start (which halves), and the Booster abilities. They are offered only to Pokémon that actually have them, and every row is labelled with its condition — a doubled Speed that needs rain on the field is a different claim from an unconditional one, so an ability row also ranks below an unconditional answer rather than being treated as a free win. Protosynthesis and Quark Drive apply only where Speed is the species' highest base stat, since they boost whichever stat is highest: Iron Bundle gets the 1.3x, Flutter Mane (highest Special Attack) does not
  ...
  - **Saved comparisons:** a whole query — both rosters, presets, modifiers, format, hidden ladder rows — can be named and kept in the browser (`localStorage`), so a league you scout every week is not retyped. Independent of the URL, which stays the sharing mechanism
  - **Ladder rows can be hidden** individually and restored with one click, for benchmarks that are just clutter in a given matchup. Hiding is display-only and says so: the row is still counted in the outspeed maths, because a benchmark you stopped looking at is still one you can lose to
